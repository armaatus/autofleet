...
#
#   ./scripts/fleet/await-review.sh            # the PR for this worktree's branch
#   ./scripts/fleet/await-review.sh 75
#
...
CONFLICT_FILE="$REPO_ROOT/.autofleet/run/conflict-head"
...
while [ "$waited" -lt "$DEADLINE_SECONDS" ]; do
  if fleet_stopped; then
    echo
    echo "STOPPED: $FLEET_STOP exists. Put the work down and report where you got to."
    exit 3
  fi
  ...
  #
  # Twice because a host-test job can drive a real service over thousands of
  # requests and a
...
if reviews and seen_head == head and seen_stamp:
    ...
    if not unseen:
        notes.append(
            ...
            "./scripts/fleet/review-status.sh, not another wait. Something left? "
