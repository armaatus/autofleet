#!/usr/bin/env bash
# Covers scripts/fleet/env.sh -- the first thing setup.sh runs, and the one step
...
#
# Stripping PATH down to the fakes alone does not work: fleet_python_candidates
# sorts, fleet_python_is_usable tails, and setup.sh's own preamble needs
...
# reports. Enough of `python3 -V` and the version_info check for fleet_pick_python
...
case "${1:-}" in
  python)
    ...
    with_fake_path
    fleet_python_is_usable python3; usable=$?
    picked="$(fleet_pick_python)"; found=$?
    restore_path
    ...
    with_fake_path
    picked="$(fleet_pick_python)"; restore_path
    [ "$picked" = python3.99 ] \
    ...
    with_fake_path
    fleet_python_is_usable python3.98
    reason="$fleet_python_reject"
    restore_path
    ...
    picked="$(fleet_pick_python)" || fail "no usable interpreter on this machine's PATH"
    fleet_python_is_usable "$picked" \
      || fail "fleet_pick_python returned $picked, which its own check rejects"
  ...
  venv)
    ...
    fleet_venv_is_usable "$FIXTURE/old" \
    ...
    real="$(fleet_pick_python)" || fail "no usable interpreter to build a real venv with"
    "$real" -m venv "$FIXTURE/good" >/dev/null 2>&1 \
      || fail "could not build a venv with $real, so the positive case is untested"
    fleet_venv_is_usable "$FIXTURE/good" \
  ...
  setup_fails_fast)
    ...
    out="$(cd "$FIXTURE" && PATH="$fake_path" "$BASH" ./scripts/fleet/setup.sh 2>&1)"
    rc=$?
  ...
  concurrent)
    ...
    for i in $(seq 1 "$WRITERS"); do
      "$FIXTURE/scripts/fleet/env.sh" >"$FIXTURE/out.$i" 2>&1 &
      pids="$pids $!"
    done
    failed=0
    ...
    grep -q "^COMPOSE_PROJECT_NAME=${AUTOFLEET_PROJECT_PREFIX:-af}-" "$FIXTURE/.env" \
    ...
    for i in $(seq 1 "$WRITERS"); do
      grep -q "project=${AUTOFLEET_PROJECT_PREFIX:-af}-" "$FIXTURE/out.$i" \
        || fail "writer $i reported no derivation: $(cat "$FIXTURE/out.$i")"
    done
    # Six is what env.sh writes with no AUTOFLEET_PORTS configured: the header
    # comment pair, the two names for the project, the slug and the offset. A
    # read that landed inside a truncating rewrite gets fewer, and the point of
    # the whole phase is that no such read is ever the file that stays.
    [ "$want" -ge 6 ] || fail ".env looks truncated ($want lines): $(cat "$FIXTURE/.env")"
  ...
  readable)
    make_fixture
    "$FIXTURE/scripts/fleet/env.sh" >/dev/null 2>&1 || fail "first run failed"
    ...
    ( for _ in $(seq 1 25); do "$FIXTURE/scripts/fleet/env.sh" >/dev/null 2>&1; done ) &
    writer=$!
    bad=0
    while kill -0 "$writer" 2>/dev/null; do
      grep -q "^COMPOSE_PROJECT_NAME=${AUTOFLEET_PROJECT_PREFIX:-af}-" "$FIXTURE/.env" 2>/dev/null || bad=$((bad + 1))
    done
