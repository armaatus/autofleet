#!/usr/bin/env bash
# Covers scripts/fleet/resolve-thread.sh -- resolving a review thread AND asking
...
make_fixture() {
  ...
  # The WHOLE payload, not the two scripts under test: lib.sh sources config.sh
  # and a runner driver, so a hand-picked subset is a fixture that cannot load.
...
run_it() { (cd "$WORK/repo" && ./scripts/fleet/resolve-thread.sh "$@"); }
