...
4. **The Orca dependency is named, not hidden.** Everything Orca-specific lives
   ...
   is a bug, not debt. `evals/lint.sh` runs the grep that says so — the pattern
   is `ORCA_\|orca\b`, wide enough to see `ORCA_DEADLINE` and `mkdir -p .orca`,
   and it skips comments (naming Orca in prose *is* this rule) and `config.sh`'s
   `AUTOFLEET_RUNNER` default (that file chooses the driver). The exact pipeline
   is in [docs/RUNNERS.md](docs/RUNNERS.md). And
   `tests/run.sh fleet runner_stub` drives the whole fleet on a driver that is
   not Orca.
